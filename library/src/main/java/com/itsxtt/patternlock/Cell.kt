...
internal class Cell(context: Context,
                    ...
                    private var cellSizeDp: Float = 0f) : View(context) {
    ...
    private fun drawDot(canvas: Canvas?,
        ...
        if (background is ColorDrawable) {
            ...
            if (cellSizeDp != 0f) {
                val left = (width - cellSizeDp).toInt() / 2
                val right = left + cellSizeDp.toInt()
                val top = (height - cellSizeDp).toInt() / 2
                val bottom = top + cellSizeDp.toInt()
                Log.d("PatternLockView", "Setting cell bounds: $left, $right, $top, $bottom")
